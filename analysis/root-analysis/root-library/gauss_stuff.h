...
public:
    ...
    static TF1* multi_gauss(TH1F* hist, int n_peaks_requested, const TString& fit_name = "fit") {
        ...
        for (auto& pk : peaks) {
            double range_min = pk.mean - 2.5;
            double range_max = pk.mean + 2.5;
            TF1 gausFit("tmpGaus", "gaus", range_min, range_max);
            gausFit.SetParameters(pk.amplitude, pk.mean, 1.);
        ...
        double fit_max = peaks.back().mean + 1*peaks.back().sigma;
    ...
    static TF1* FitQGaussPlusConst(TH1* proj, const char* fitName) {
        ...
        qGaussPlusConst->SetParLimits(4, 1.0, 2.0);
