...
public:
    ...
    static SignalBackground CalculateSignalAndBackground(TH1D* hist, int b) {
        ...
        // Centro del bin in tempo
        double center_time = xaxis->GetBinCenter(b);

        // ---- Finestre temporali (ns)
        double S_min = center_time - 3;
        double S_max = center_time + 3;

        double B_min = center_time - 9;
        double B_max = center_time - 3;

        // ---- Conversione tempo -> bin
        int S_start = std::max(1, xaxis->FindBin(S_min));
        int S_end   = std::min(nBins, xaxis->FindBin(S_max));

        int B_start = std::max(1, xaxis->FindBin(B_min));
        int B_end   = std::min(nBins, xaxis->FindBin(B_max));

        // ---- Debug
        std::cout << "Bin centrale: " << b
                  << " (x: " << center_time << " ns)\n";

        std::cout << "Signal time: [" << S_min << ", " << S_max
                  << "] -> bins [" << S_start << ", " << S_end << "]\n";

        std::cout << "Background time: [" << B_min << ", " << B_max
                  << "] -> bins [" << B_start << ", " << B_end << "]\n";

        // ---- Integrali grezzi
        double S = hist->Integral(S_start, S_end);
        double Braw = hist->Integral(B_start, B_end);

        // ---- Numero di bin
        int nSbins = S_end - S_start + 1;
        int nBbins = B_end - B_start + 1;

        // ---- Fondo stimato sotto il segnale (scalato)
        double B = Braw * (double)nSbins / (double)nBbins;

        std::cout << "Signal count: " << S
                  << ", Raw background: " << Braw
                  << ", Scaled background under signal: " << B << "\n";
