...
public:
    ...
    std::vector<std::pair<std::vector<double>, std::vector<double>>> getWaveforms(
            ...
            Double_t* amp,
            bool align = true,
            bool invertAmplitude = true) const
    {
        ...
        for (int i = 0; i < n_events; ++i) {
            ...
            for (int j = 0; j < waveform_size; ++j) {
                ...
                a_vec[j] = invertAmplitude ? -amp[j] : amp[j];
            }
    ...
    correctWaveforms(
        ...
        for (const auto& [time, amp] : waveforms) {
            ...
            // --- quality cut opzionale ---
            if (apply_quality_cut) {

                // 1) ricalcolo baseline residua sul pre-signal DOPO sottrazione
                double post_baseline = 0.0;
                int n_pre = 0;

                for (size_t i = 0; i < time.size(); ++i) {
                    if (time[i] <= pre_signal_end) {
                        post_baseline += corrected_amp[i];
                        n_pre++;
                    }
                }
                ...
                // 2) peak nella finestra 0–30 ns dopo pre_signal_end
                double max_p = -1e300;
                double min_p =  1e300;

                for (size_t i = 0; i < time.size(); ++i) {

                    if (time[i] <= pre_signal_end) continue;
                    if (time[i] > pre_signal_end + 30.0) break;

                    max_p = std::max(max_p, corrected_amp[i]);
                    min_p = std::min(min_p, corrected_amp[i]);
                }

                double peak = std::max(std::abs(max_p - post_baseline),
                                       std::abs(min_p - post_baseline));

                // 3) criterio di scarto (ora coerente)
                if (peak > 2.0 * std::abs(post_baseline)) {
                    continue;
                }
            }
...
private:
    ...
    void computeLaserCrossings() {
        ...
        for (int i = 0; i < n_events; ++i) {
            ...
            double t_laser = TimeStuff::computeCrossingTime(laser_time, laser_amp,
                                                            waveform_size, 70);
