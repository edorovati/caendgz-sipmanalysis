...
class waveform_analysis:
    ...
    @staticmethod
    def analyze_waveforms(wf_data, fs, baseline_range, p2p_threshold=0, max_waveforms=100, unit='mV', get_edges=False):
        """
        Filters and preprocesses waveform data.

        Returns:
        - selected_waveforms: list of filtered waveforms
        - rising_edges: list of rising edge indices (only if get_edges=True)
        """
        ...
        for key in wf_data.files:
            ...
            if wf_array.ndim == 2 and wf_array.shape[0] > 0:
                for i in range(wf_array.shape[0]):
                    ...
                    if valid_waveforms_count >= max_waveforms:
                        break
            if valid_waveforms_count >= max_waveforms:
                break

        if get_edges:
            return selected_waveforms, rising_edges
        else:
            return selected_waveforms



    def persistence(self, npz_path, unit='mV', p2p_threshold=0, num_waveforms=100, color='green', align="auto"):
        ...
        filename_info += f"Sampling rate: {info["sampling_rate_hz"]/ 1e9} GS\n"
