...
###########################
    @staticmethod
    def store_avg(npz_path, unit='mV', amplitude_threshold=4, num_waveforms=100,
                color='green', align="False", sampling=None, output_filename=None, batch = False):
        ...
        # === PRELIMINARY - 1 pe peak selection via clustering (better to use Roberto's code?) ===
        try:
            kmeans = KMeans(n_clusters=2, random_state=0)
            kmeans.fit(peak_values)
        except ValueError as e:
            print(f"⚠️ KMeans fallito: {e}")
            print("⏭️  Skipping file due to lack of valid peak values.")
            return [], []
        ...
        if align:
            ...
            # === selection around the 150-th frame (aligned-peak) ===
        ...
        # === SINGLE-PEAK SELECTION - PRELIMINARY! ===
        ...
        if batch == True:
            batches = [
                single_peak_waveforms[i:i + batch_size]
                for i in range(0, len(single_peak_waveforms), batch_size)
            ]
            #le'ts APPEND the avg waveform to the total vector
            if len(batches) == 0:
                print("⚠️ Nessun batch creato! Controlla il numero di waveform selezionate.")
                return [], []
            print(f"✅ Creati {len(batches)} batch di dimensione {batch_size} per il calcolo della media.")
            # Calcola la media per ogni batch
            
            for batch_idx, batch_waveforms in enumerate(batches):
                avg_wf = np.median(batch_waveforms, axis=0)
                if batch_idx == 0:
                    avg_waveforms = [avg_wf]
                else:
                    avg_waveforms.append(avg_wf)
            

            print(f"✅ Calcolate {len(avg_waveforms)} medie su gruppi da {batch_size} waveform")

            taus = []
            for idx, avg_wf in enumerate(avg_waveforms):
                dt = 1e3 / sampling  # ns per sample
                ...
                for i in range(peak_idx + 1, len(avg_wf)):
                    if avg_wf[i] <= target_val:
                        ...
                        break
                if tau_ns is not None:
                    taus.append((tau_ns, peak_val))
                    print(f"[{idx}] Tau stimato: {tau_ns:.3f} ns")
                else:
                    print(f"[{idx}] ⚠️ Nessun crossing trovato per 1/e decay!")
            ...
            if taus:
                ...
                with open(output_filename, "w") as f:
                    for tau, peak in taus:
                        f.write(f"{tau:.6f}\t{peak:.6f}\n")
                print(f"✅ Salvate {len(taus)} righe in {output_filename}")
    ...
    def persistence2(npz_path, unit='mV', amplitude_threshold=4, num_waveforms=100,
        ...
        # Calcola la deviazione standard punto per punto
        waveforms_array = np.array(single_peak_waveforms)
        point_std = np.std(waveforms_array, axis=0, ddof=1)
        err_mean = point_std / np.sqrt(len(single_peak_waveforms))
        ...
        for i in range(peak_idx + 1, len(avg_wf)):
            if avg_wf[i] <= target_val:
                ...
                # Calcola la pendenza (usando differenze finite)
                slope = abs(avg_wf[i] - avg_wf[i - 1])
                if slope < 1e-12:
                    slope = 1e-12  # Evita divisione per zero

                # Calcola l'errore sul tau propagato
                tau_err_ns = dt * (err_mean[i] / slope)
                break

        taus = []
        if tau_ns is not None:
            taus.append((tau_ns, tau_err_ns, peak_val))
        ...
        if taus:
            ...
            with open(output_filename, "w") as f:
                ...
                for tau, tau_err, peak in taus:
                    f.write(f"{tau:.6f}\t{tau_err:.6f}\t{peak:.6f}\n")
